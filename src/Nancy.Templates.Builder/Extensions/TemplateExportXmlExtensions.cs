...
namespace Nancy.Templates.Builder.Extensions
{
    ...
    static class TemplateExportXmlExtensions
    {
        ...
        public static void AddVsMyTemplate(
            this Template   template,
            ZipArchive      archive
            )
        {
            var xVsTemplate = template.ToVsTemplate();
            
            var entry = archive.CreateEntry(VsTemplate, CompressionLevel.Optimal);
            using (var target = entry.Open())
            {
                using (var xw = XmlWriter.Create(
                                                    target,
                                                    VsTemplateXmlWriterSettings
                                                )
                    )
                {
                    xVsTemplate.Save(
        ...
        private static XElement ToVsTemplate(
            this Template template
            )
        {
            ...
            var xVsTemplate = XElement.Parse(TemplateXElement.Root);
            xVsTemplate.Add(
                new XElement(
